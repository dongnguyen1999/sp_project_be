...
from .models import TrnQuestPart, TrnQuest, Student, QuestionAns, QuestionPartAns, Class, Question, QuestionPart
            ...
            try:
                ...
                for row in range(data.shape[0]):
                    ...
                    for i in range(0, no_question_cols):
                        ...
                        if pd.isna(partid):
                            temp = (questionid, '')
                            if temp not in questions_pool:
                                questions_pool.append(temp)
                                question = data.iat[row, index_temp + 2]
                                question_table = Question(questionid=questionid, question=question, partstatus=0,
                                                          activestatus=1)
                                try:
                                    with transaction.atomic():
                                        question_table.save()
                                        dict_json['Question'].append({'questionid': int(questionid)})
                                except IntegrityError:
                                    transaction.rollback()
                                    error_code = 1
                                    print("2 \n")
                            ...
                            temp = (questionid, partid)
                            if temp not in questions_pool:
                                questions_pool.append(temp)
                                question = data.iat[row, index_temp + 2]
                                if Question.objects.filter(questionid=questionid).count() == 0:
                                    question_table = Question(questionid=questionid,
                                                                      question=question, partstatus=1, activestatus=1)
                                    try:
                                        with transaction.atomic():
                                            question_table.save()
                                            dict_json['Question'].append({'questionid': int(questionid)})
                                ...
                                questionpart_table = QuestionPart(questionid=questionid, partid=partid,
                                                                  partquestion=question)
                                try:
                                    with transaction.atomic():
                                        questionpart_table.save()
                                        dict_json['QuestionPart'].append(
                                            {'questionid': int(questionid), 'partid': partid})
                                except IntegrityError:
                                    error_code = 1
                                    transaction.rollback()
                                    print("4 \n")
                        ...
                        if pd.isna(partid):
                            ...
                            trnquestpart = TrnQuestPart(studid=studid, questionid=questionid, partid=partid,
                                                        partresponse=response)
                            try:
                                with transaction.atomic():
                                    trnquestpart.save()
                                    dict_json['TrnQuestPart'].append(
                                        {'studid': int(studid), 'questionid': int(questionid), 'partid': partid})
                            except IntegrityError:
                                error_code = 1
                                error_line.add(row + 1)
                                transaction.rollback()
    ...
    if not file.name.endswith(('.xlsx', '.xls')):
        ...
        for row in range(data_filtered.shape[0]):
            ...
            if ansid_temp == 0:
                ...
                ansid_temp = QuestionAns.objects.filter(questionid=questionid).aggregate(Max('ansid'))['ansid__max'] + 1
        ...
        for row in range(data_filtered.shape[0]):
            ...
            if ansid_temp == 0:
                ...
                ansid_temp = \
                    QuestionPartAns.objects.filter(questionid=questionid, partid=partid).aggregate(Max('partansid'))[
                        'partansid__max'] + 1
            modelans = data_filtered.iat[row, 3]
            ansmark = data_filtered.iat[row, 4]
            questionpartans = QuestionPartAns(questionid=questionid, partid=partid, partansid=ansid_temp,
                                              modelans=modelans, ansmark=ansmark)
            try:
                with transaction.atomic():
                    questionpartans.save()
                    dict_json['QuestionPartAns'].append(
                        {'questionid': int(questionid), 'partid': partid, 'partansid': ansid_temp})
            except IntegrityError:
                error_code = 1
                error_line.add(row + 1)
                transaction.rollback()
